...
class JvmMethodMangler {
  ...
  resolveTarget (wrapper, isInstanceMethod, env, api) {
    ...
    const vip = originalMethod.oldMethod.vtableIndexPointer;
...
function installJvmMethod (method, methodId, vm) {
  ...
  old.accessFlagsPointer.writeU32((old.accessFlags | JVM_ACC_IS_OLD | JVM_ACC_IS_OBSOLETE) >>> 0);
  ...
  const doKlassPointer = Memory.alloc(pointerSize);
  doKlassPointer.writePointer(api.doKlass);
  klassClosure.writePointer(doKlassPointer);
  ...
  if ('ResolvedMethodTable::adjust_method_entries' in api) {
    api['ResolvedMethodTable::adjust_method_entries'](traceNamePrinted);
  } else if (!method.memberNames.isNull() &&
            'MemberNameTable::adjust_method_entries' in api) {
    api['MemberNameTable::adjust_method_entries'](method.memberNames, method.instanceKlass, traceNamePrinted);
  }
}
...
function readJvmMethod (method, constMethod, constMehtodSize, vm) {
  ...
  const constPointer = method.add(spec.method.constMethodOffset);
  const accessFlagsPointer = method.add(spec.method.accessFlagsOffset);
  ...
  const vtableIndexPointer = method.add(spec.method.vtableIndexOffset);
  const vtableIndex = vtableIndexPointer.readS32();
