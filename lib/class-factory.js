...
function ClassFactory (vm) {
  ...
  const xposedIsSupported = getAndroidApiLevel() < 28;
  let loader = null;
  ...
  function registerClass (spec) {
    ...
    try {
      ...
      const dexSpec = {
        ...
        superClass: spec.superClass ? makeJniObjectTypeName(spec.superClass) : 'Ljava/lang/Object;',
      ...
      interfaces.forEach(iface => {
        ...
        Object.getOwnPropertyNames(ifaceProto)
          .filter(name => {
            return name[0] !== '$' && name !== 'constructor' && name !== 'class' && iface[name].overloads;
          })
          .forEach(name => {
      ...
      if (spec.superClass) {
        let candidates = null;
        const superMethods = function (self) {
          this.$holder = self;
          if (candidates === null) {
            candidates = {};
            let superProto = Klass.$classWrapper.__super__;
            while (superProto) {
              Object.getOwnPropertyNames(superProto)
                  .filter(name => {
                    try {
                      return name[0] !== '$' && name !== 'constructor' && name !== 'class' && superProto[name].overloads;
                    } catch (e) {
                      return false; // access instance field will throw error
                    }
                  })
                  .forEach(name => {
                    const overloads = superProto[name].overloads.filter(overload => overload.type === INSTANCE_METHOD);
                    if (!candidates.hasOwnProperty(name)) {
                      candidates[name] = overloads;
                    } else{
                      [].push.apply(candidates[name], overloads);
                    }
                  });

              const superKlass = new superProto.constructor(null);
              superProto = superKlass.$classWrapper.__super__;
            }

            for (let name in candidates) {
              const overloads = candidates[name];
              Object.defineProperty(superMethods.prototype, name, {
                enumerable: true,
                value: function (...args) {
                  for (let i = 0; i !== overloads.length; i++) {
                    ...
                    if (method.canInvokeWith(args)) {
                      ...
                      try {
                        return method.apply(this.$holder, args);
                      } finally {
                        method[PENDING_CALLS].delete(tid);
                      }
                    }
                  }
                  throwOverloadError(name, overloads, 'argument types do not match any of:');
                }
              });
            }
          }
        ...
        Object.defineProperty(Klass.$classWrapper.prototype, '$super', {
          ...
          get: function () {
            if (this.$handle === undefined) {
              throw new Error('Cannot access super methods without an instance');
            }
            return new superMethods(this);
          }
        });
      }
