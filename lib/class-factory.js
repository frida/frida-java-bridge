...
function ClassFactory (vm) {
  ...
  function ensureClass (getClassHandle, name) {
    ...
    klass.className = basename(name);

    function initializeClass (proto) {
      klass.__name__ = name;
      ...
      proto.$isSameObject = function (obj) {
        const env = vm.getEnv();
        return env.isSameObject(obj.$handle, this.$handle);
      };

      Object.defineProperty(proto, 'class', {
        get: function () {
          const env = vm.getEnv();
          const classHandle = this.$getClassHandle(env);
          try {
    ...
    function makeMethod (methodName, type, methodId, retType, argTypes, env, invocationOptions) {
      ...
      f.clone = options => {
        return makeMethod(methodName, type, methodId, retType, argTypes, env, options);
      };
    ...
    initializeClass(klass.prototype);
