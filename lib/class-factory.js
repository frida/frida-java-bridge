...
function ClassFactory (vm) {
  ...
  function registerClass (spec) {
    ...
    methodNames.forEach((name, index) => {
      const impl = methods[name];

      const ifaceProto = pendingMethods[name];
      if (ifaceProto === undefined) {
        throw new Error('Unexpected method not declared in any interface: ' + name);
      }
      delete pendingMethods[name];

      let method = ifaceProto[name];
      ...
      const argumentTypeNames = method.argumentTypes.map(t => t.name);

      dexMethods.push([name, returnTypeName, argumentTypeNames]);
