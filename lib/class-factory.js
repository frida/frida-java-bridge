...
function ClassFactory (vm) {
  ...
  function chooseObjectsArtModern (env, thread, className, callbacks) {
    ...
    const scope = new VariableSizedHandleScope(thread);
    ...
    const instances = new HandleVector();
  ...
  class FixedSizeHandleScope extends BaseHandleScope {
    constructor (numReferences) {
      ...
      this._refsStorage = storage.add(refsStorageOffset);
      this._pos = storage.add(posOffset);
    }

    newHandle (object) {
      const pos = Memory.readU32(this._pos);
      const handle = this._refsStorage.add(pos * 4);
      Memory.writeS32(handle, object.toInt32());
      Memory.writeU32(this._pos, pos + 1);
      return handle;
    }
  }
