...
function ClassFactory (vm) {
  ...
  function chooseObjectsArtModern (env, thread, className, callbacks) {
    ...
    const instanceHandles = instances.objects().map(object => addGlobalReference(vmHandle, thread, object));
  ...
  const BASE_HANDLE_SCOPE_SIZE = pointerSize + 4;
  ...
  class BaseHandleScope {
    constructor (storage) {
      ...
      this._link = storage.add(BASE_HANDLE_SCOPE_OFFSET_LINK);
      this._numberOfReferences = storage.add(BASE_HANDLE_SCOPE_OFFSET_NUM_REFS);
    }
  ...
  const VSHS_OFFSET_SELF = alignPointerOffset(BASE_HANDLE_SCOPE_SIZE);
  ...
  class VariableSizedHandleScope extends BaseHandleScope {
    static $new (thread) {
      ...
      const scopeLayout = FixedSizeHandleScope.layoutForCapacity(kNumReferencesPerScope);

      const scope = new VariableSizedHandleScope(api.$new(VSHS_SIZE), scopeLayout, topHandleScopePtr);

      scope.link = Memory.readPointer(topHandleScopePtr);
      scope.numberOfReferences = kNumReferencesVariableSized;

      scope.self = thread;
      scope.currentScope = FixedSizeHandleScope.$new(scopeLayout);

      Memory.writePointer(topHandleScopePtr, scope);

      return scope;
    }

    constructor (storage, scopeLayout, topHandleScopePtr) {
      super(storage);

      this._self = storage.add(VSHS_OFFSET_SELF);
      this._currentScope = storage.add(VSHS_OFFSET_CURRENT_SCOPE);

      this._scopeLayout = scopeLayout;
    ...
    get currentScope () {
      return new FixedSizeHandleScope(Memory.readPointer(this._currentScope), this._scopeLayout);
    }
  ...
  class FixedSizeHandleScope extends BaseHandleScope {
    static layoutForCapacity (numRefs) {
      const refsStorage = BASE_HANDLE_SCOPE_SIZE;
    ...
    static $new (layout) {
      const storage = api.$new(layout.size);
      puts('FixedSizeHandleScope ' + storage);
      puts('layout=' + JSON.stringify(layout));

      const scope = new FixedSizeHandleScope(storage, layout);

      scope.link = NULL;
      scope.numberOfReferences = layout.numberOfReferences;
    ...
    $delete () {
      api.$delete(this.handle);
      puts('~FixedSizeHandleScope ' + this.handle);
    }
  ...
  class StdVector {
    static $new (elementSize) {
      return new StdVector(api.$new(STD_VECTOR_SIZE), elementSize);
    }
    ...
    destroy () {
      const data = Memory.readPointer(this._begin);
      api.$delete(data);
      puts('deleted data=' + data);
    }

    get size () {
      return Memory.readPointer(this._end).sub(Memory.readPointer(this._begin)).toInt32() / this._elementSize;
    }
  }

  class HandleVector extends StdVector {
    static $new () {
      return new HandleVector(api.$new(STD_VECTOR_SIZE));
    }
    ...
    get objects () {
      ...
      while (!cur.equals(end)) {
        result.push(ptr(Memory.readU32(Memory.readPointer(cur))));
