            ...
            return new Proxy(superInstance, {
              get: function (target, property, receiver) {
                ...
                return new Proxy(prop, {
                  apply: function (target, thisArg, args) {
                    const method = prop.overloads.find(m => m.canInvokeWith(args));
                    if (!method) {
                      throwOverloadError(property, prop.overloads, 'argument types do not match any of:');
                    }
                    return proxy(method).apply(target, args);
                  },
                  get: function (target, property, receiver) {
                    switch (property) {
                      case 'overloads':
                        return prop.overloads.map(proxy);
                      case 'overload':
                        return function (...args) {
                          return proxy(prop.overload.apply(superInstance, args))
                        };
                      default:
