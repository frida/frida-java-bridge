...
function ClassFactory (vm) {
  ...
  function makeMember (name, spec, classHandle, classWrapper, env) {
    ...
    if (type === 'm') {
      return makeMethodFromParams(name, params, classHandle, classWrapper, env);
    }

    if (type === 'f') {
      return makeFieldFromParams(name, params, classHandle, classWrapper, env);
    }
  }

  function makeMethodFromParams (name, params, classHandle, classWrapper, env) {
    const overloads = params
        .map(p => {
          const [id, modifiers] = p.split(',');
          return [ptr(id), parseInt(modifiers)];
        });
    ...
    const methods = overloads.map(function (params) {
      const [rawMethodId, modifiers] = params;
      const methodId = ptr(rawMethodId);
...
`
    ...
    f.clone = options => {
      return makeMethod(methodName, type, methodId, retType, argTypes, classWrapper, env, options);
    };
  ...
  function makeFieldFromParams (name, params, classHandle, classWrapper, env) {
    let intermediates = (function () {
      const [rawId, rawType] = params[0].split(',');
      const id = ptr(rawId);
      ...
      let rtype;
      const isStatic = (type === STATIC_FIELD) ? 1 : 0;
      const handle = env.toReflectedField(classHandle, id, isStatic);
      try {
