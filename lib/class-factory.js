...
function ClassFactory (vm) {
  ...
  this.choose = function (specifier, callbacks) {
    if (api.flavor === 'art') {
      if (api['art::gc::Heap::VisitObjects'] === undefined) {
        chooseObjectsArtModern(specifier, callbacks);
      } else {
        chooseObjectsArtLegacy(specifier, callbacks);
      }
    } else {
  ...
  function chooseObjectsArtModern (className, callbacks) {
    const env = vm.getEnv();
    const klass = factory.use(className);

    const threadHandle = getArtThreadFromEnv(env);

    const scope = new VariableSizedHandleScope(threadHandle);

    const localClassHandle = klass.$getClassHandle(env);
    const globalClassHandle = env.newGlobalRef(localClassHandle);
    const needle = api['art::JavaVMExt::DecodeGlobal'](api.vm, threadHandle, globalClassHandle);
    env.deleteGlobalRef(globalClassHandle);
    env.deleteLocalRef(localClassHandle);

    const maxCount = 0;

    const instances = new StdVector(pointerSize);

    api['art::gc::Heap::GetInstances'](api.artHeap, scope, needle, maxCount, instances);

    console.log('size=' + instances.size());

    instances.dispose();
    scope.dispose();
  }

  const BASE_HANDLE_SCOPE_SIZE = 2 * pointerSize;
  const BASE_HANDLE_SCOPE_OFFSET_LINK = 0;
  const BASE_HANDLE_SCOPE_OFFSET_NUM_REFS = pointerSize;

  const kNumReferencesVariableSized = -1;

  class BaseHandleScope {
    constructor (storage, link, numRefs) {
      this.handle = storage;

      this.link = link;

      Memory.writePointer(storage.add(BASE_HANDLE_SCOPE_OFFSET_LINK), link);
      Memory.writeS32(storage.add(BASE_HANDLE_SCOPE_OFFSET_NUM_REFS), numRefs);
    }
  ...
  class VariableSizedHandleScope extends BaseHandleScope {
    constructor (thread) {
      const selfOffset = BASE_HANDLE_SCOPE_SIZE;
      const curScopeOffset = selfOffset + pointerSize;
      const size = curScopeOffset + pointerSize;

      const storage = Memory.alloc(size);

      const topHandleScopeOffset = getArtThreadSpec(vm).offset.topHandleScope;
      const topHandleScopePtr = thread.add(topHandleScopeOffset);
      const link = Memory.readPointer(topHandleScopePtr);

      super(storage, link, kNumReferencesVariableSized);
      ...
      const currentScope = new FixedSizeHandleScope(kNumReferencesPerScope);
      this.currentScope = currentScope;

      Memory.writePointer(storage.add(selfOffset), thread);
      Memory.writePointer(storage.add(curScopeOffset), currentScope);

      Memory.writePointer(topHandleScopePtr, this.handle);
      this.topHandleScopePtr = topHandleScopePtr;
    }

    dispose () {
      Memory.writePointer(this.topHandleScopePtr, this.link);
  ...
  class FixedSizeHandleScope extends BaseHandleScope {
    constructor (numReferences) {
      const refsStorageOffset = BASE_HANDLE_SCOPE_SIZE;
      const posOffset = refsStorageOffset + (numReferences * 4);
      const size = posOffset + 4;

      const storage = Memory.alloc(size);

      super(storage, NULL, numReferences);
    }
  }

  class StdVector {
    constructor (elementSize) {
      const block = Memory.alloc(3 * pointerSize);

      this.handle = block;

      this.begin = block;
      this.end = block.add(pointerSize);
      this.storage = block.add(2 * pointerSize);
    ...
    size () {
      return Memory.readPointer(this.end).sub(Memory.readPointer(this.begin)).toInt32() / this.elementSize;
    }
  }

  function chooseObjectsArtLegacy (className, callbacks) {
    const env = vm.getEnv();
    const klass = factory.use(className);
