    ...
    function makeMethod (methodName, type, methodId, retType, argTypes, env) {
      ...
      function replaceDalvikImplementation (fn) {
        ...
        if (fn !== null) {
          ...
          Memory.writeU32(methodId.add(DVM_METHOD_OFFSET_JNI_ARG_INFO), computeJniArgInfo(methodId));
      ...
      function computeJniArgInfo(methodId) {
        if (Process.arch !== 'ia32') {
              return DALVIK_JNI_NO_ARG_INFO;
        }

        //For the 386 ABI, valid hints should always be generated
        let shorty = Memory.readCString(Memory.readPointer(methodId.add(DVM_METHOD_OFFSET_SHORTY)));
        let returnType;
        let hints = 0;
        if (!shorty || !shorty.length) {
          return DALVIK_JNI_NO_ARG_INFO;
        }
        if (shorty.length > 0xFFFF) {
          return DALVIK_JNI_NO_ARG_INFO; // Invalid - Dex file limitation
        }
        switch (shorty[0]) {
          case 'V':
        ...
        for (let i = shorty.length - 1; i > 0; --i) {
          hints += (shorty[i] === 'D' || shorty[i] === 'J') ? 2 : 1;
        }
