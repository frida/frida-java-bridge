...
let wrapperHandler;
let dispatcherPrototype;
let methodPrototype;
let valueOfPrototype;
...
class ClassFactory {
  ...
  _chooseObjectsDalvik (className, env, callbacks) {
    const classWrapper = this.use(className);
    ...
    if (api.addLocalReference === null) {
      ...
      let pattern;
      if (android.getAndroidVersion().indexOf('4.2.') === 0) {
        // Verified with 4.2.2
      ...
      enumerateInstances(env);
    }

    function enumerateInstances (env) {
        ...
        onMatch: (address, size) => {
          if (api.dvmIsValidObject(address)) {
            vm.perform(() => {
              ...
              try {
                instance = this.cast(localReference, classWrapper);
              } finally {
...
function makeConstructor (classHandle, classWrapper, env) {
  const {$className: className, $f: factory} = classWrapper;
    ...
    if (n > 0) {
      ...
      try {
        for (let i = 0; i !== n; i++) {
          ...
          const jsArgTypes = readTypeNames(env, types).map(name => factory._getType(name));
...
function makeMethodFromSpec (name, spec, classHandle, classWrapper, env) {
  ...
  try {
    methods = overloads.map(params => {
      ...
      return makeMethod(name, classWrapper, jsType, methodId, jsRetType, jsArgTypes, env);
    })
    .filter(m => m !== null);
  } finally {
    env.popLocalFrame(NULL);
    ...
    value (receiver, args) {
      ...
      try {
        ...
        if (replacement === undefined) {
          ...
          methodId = mangler.resolveTarget(vm);
          ...
          if (isArtVm) {
            ...
            mangler.synchronizeVtable(receiver, isInstanceMethod, env, api);
          }
        }
        ...
        return retType.fromJni(jniRetval, env);
      } finally {
...
function handleMethodInvocation (jniArgs, params) {
  ...
  try {
    ...
    for (let i = 0; i !== numArgs; i++) {
      args.push(argTypes[i].fromJni(jniArgs[2 + i], env));
    }
...
valueOfPrototype = Object.create(Function.prototype, {
  ...
  returnType: {
    ...
    get () {
      ...
      return classWrapper.$f.use(classWrapper.$className);
    }
  },
  argumentTypes: {
    enumerable: true,
  ...
  value: {
    ...
    get () {
      ...
      try {
        ...
        return rtype.fromJni(jniRetval, env);
      } finally {
        if (borrowedHandle !== null) {
          borrowedHandle.unref(env);
        }
...
class DexFile {
  ...
  load () {
    ...
    factory.loader = DexClassLoader.$new(file.getCanonicalPath(), factory.cacheDir, null, factory.loader);
