...
class CodeAllocator {
  ...
  _isSliceNear (slice, spec) {
    ...
    const startDistance = int64(abs(near.sub(slice)).toString());
    const endDistance = int64(abs(near.sub(sliceEnd)).toString());
