...
const safeFunctionOptions = {
  scheduling: 'exclusive',
  exceptions: 'propagate'
};

const unsafeFunctionOptions = {
  exceptions: 'propagate'
};
...
function proxy (offset, retType, argTypes, wrapper) {
  ...
  return function () {
    if (impl === null) {
      impl = new NativeFunction(Memory.readPointer(vtable(this).add(offset * pointerSize)), retType, argTypes, safeFunctionOptions);
    }
    let args = [impl];
