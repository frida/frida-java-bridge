...
function plainMethod (offset, retType, argTypes) {
  ...
  if (!m) {
    ...
    m = new NativeFunction(vtable(this).add(offset * pointerSize).readPointer(), retType, ['pointer', 'pointer', 'pointer'].concat(argTypes),
...
function nonvirtualVaMethod (offset, retType, argTypes) {
  ...
  if (!m) {
    /* jshint validthis: true */
    m = new NativeFunction(vtable(this).add(offset * pointerSize).readPointer(), retType, ['pointer', 'pointer', 'pointer', 'pointer', '...'].concat(argTypes),
        nativeFunctionOptions);
    cachedVaMethods[key] = m;
  }
  return m;
