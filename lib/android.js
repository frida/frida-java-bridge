...
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_IA32 = 5;
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_X64 = 14; // TODO
    ...
    if (value.equals(internTable)) {
      ...
      spec = {
        offset: {
          ...
          quickToInterpreterBridgeTrampoline: offset + ((delta + 1) * pointerSize)
        }
      };
...
function notifyArtMethodHooked (method, vm) {
  ensureArtKnowsHowToHandleHookedMethods(vm);
}
...
function ensureArtKnowsHowToHandleHookedMethods (vm) {
  ...
  const api = getApi();
  const methodOffsets = getArtMethodSpec(vm).offset;
  ...
  const filter = generateFilterFunc(original, replacement, methodOffsets.quickCode,
    api.artClassLinker.quickGenericJniTrampoline);
  ...
  /*
   * Instrument quick entrypoints/ trampolines that bridge from the quick code
   * to a different ABI (either JNI or interpreter) in order to handle methods
   * compiled with their quick entrypoints set to one of the trampolines.
   */
  const quickTrampolines = [
    api.artQuickGenericJniTrampoline,
    api.artQuickToInterpreterBridge
  ];

  quickTrampolines.forEach(trampoline => {
    let interceptor = new ArtQuickCodeInterceptor(trampoline);
  ...
  /*
   * Instrument method invocation from interpreted code.
   */
  const artInterpreterDoCallExports = Module.enumerateExports('libart.so')
  ...
  artInterpreterDoCallExports.forEach(exp => {
    Interceptor.attach(exp.address, function (args) {
      const replacementMethod = hookedMethods.get(args[0].toString());
...
function findArtMethodReplacement (method, thread) {
  const replacementMethod = hookedMethods.get(method.toString());
  if (replacementMethod === undefined) {
    return NULL;
  }
   ...
   * Stack check.
   *
   * Returns NULL to indicate that the original method should be invoked, otherwise
   * returns a pointer to the replacement ArtMethod.
   *
   * If the caller is our own JNI replacement stub, then a stack transition
   * must have been pushed onto the current thread's linked list.
   * Therefore, we invoke the original method if the following conditions are met:
   *   1- The current managed stack is empty.
   *   2- The ArtMethod * inside the linked managed stack's top quick frame is the
   *      same as our replacement.
   */
  const threadOffsets = getArtThreadSpec().offset;

  const managedStack = thread.add(threadOffsets.managedStack);
  if (!managedStack.readPointer().isNull()) {
    return replacementMethod;
  }

  const linkManagedStack = managedStack.add(pointerSize).readPointer();
  if (linkManagedStack.isNull()) {
    return replacementMethod;
  }

  const topQuickFrame = linkManagedStack.readPointer().and(~1);
  if (topQuickFrame.isNull() || !topQuickFrame.readPointer().equals(replacementMethod)) {
    return replacementMethod;
  }
...
const findArtMethodReplacementImpl = new NativeCallback(findArtMethodReplacement, 'pointer', ['pointer', 'pointer']);
...
function writeArtQuickCodeReplacementTrampolineIA32 (trampoline, target, vm) {
  ...
  Memory.patchCode(trampoline, 128, code => {
    ...
    writer.putAndRegU32('esp', ~(16 - 1));
    writer.putSubRegImm('esp', 512);
    writer.putBytes(fxsave);

    writer.putMovRegFsU32Ptr('ebx', threadOffsets.self);
    writer.putCallAddressWithAlignedArguments(findArtMethodReplacementImpl, ['eax', 'ebx']);

    writer.putTestRegReg('eax', 'eax');
    writer.putJccShortLabel('je', 'restore_registers', 'no-hint');

    const eaxRegStackFrameOffset = 7 * pointerSize;
    writer.putMovRegOffsetPtrReg('ebp', eaxRegStackFrameOffset, 'eax');
...
function writeArtQuickCodeReplacementTrampolineX64 (trampoline, target, vm) {
  const threadOffsets = getArtThreadSpec(vm).offset;
  const artMethodOffsets = getArtMethodSpec(vm).offset;
  ...
  Memory.patchCode(trampoline, 128, code => {
    ...
    writer.putAndRegU32('rsp', ~(16 - 1));
    writer.putSubRegImm('rsp', 512);
    writer.putBytes(fxsave);

    writer.putMovRegGsU32Ptr('rbx', threadOffsets.self);
    writer.putCallAddressWithAlignedArguments(findArtMethodReplacementImpl, ['rdi', 'rbx']);

    writer.putTestRegReg('rax', 'rax');
    writer.putJccShortLabel('je', 'restore_registers', 'no-hint');

    const rdiRegStackFrameOffset = 8 * pointerSize;
    writer.putMovRegOffsetPtrReg('rbp', rdiRegStackFrameOffset, 'rax');
...
function writeArtQuickCodePrologueX86 (target, trampoline) {
  Memory.patchCode(target, 128, code => {
...
class ArtQuickCodeInterceptor {
  ...
  activate (vm) {
    ...
    this.overwrittenPrologue = Memory.dup(this.quickCode, prologueLength);

    const writePrologue = artQuickCodePrologueWriters[Process.arch];
    writePrologue(this.quickCode, this.trampoline);
  }

  deactivate () {
    const Writer = thunkWriters[Process.arch];

    const length = this.overwrittenPrologueLength;
    Memory.patchCode(this.quickCode, length, code => {
      const writer = new Writer(code, { pc: this.quickCode });
      writer.putBytes(this.overwrittenPrologue.readByteArray(length));
      writer.flush();
    });
  }
}

function isArtQuickTrampoline (address) {
  const api = getApi();

  return address.equals(api.artClassLinker.quickGenericJniTrampoline) ||
      address.equals(api.artQuickGenericJniTrampoline) ||
      address.equals(api.artClassLinker.quickToInterpreterBridgeTrampoline) ||
      address.equals(api.artQuickToInterpreterBridge);
}
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    if ((this.originalMethod.accessFlags & kAccXposedHookedMethod) !== 0 && xposedIsSupported()) {
    ...
    const originalFlags = hookedMethodId.add(artMethodOffsets.accessFlags).readU32();
    ...
    patchArtMethod(hookedMethodId, {
      accessFlags: (originalFlags & ~(kAccFastInterpreterToInterpreterInvoke | kAccSkipAccessChecks)) >>> 0
    }, vm);

    hookedMethods.set(hookedMethodId.toString(), replacementMethodId);

    notifyArtMethodHooked(replacementMethodId, vm);

    const quickCode = this.originalMethod.quickCode;
    if (!isArtQuickTrampoline(quickCode)) {
  ...
  revert (vm) {
    hookedMethods.delete(this.hookedMethodId.toString());
