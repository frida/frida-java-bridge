    ...
    while (pending.length > 0) {
      ...
      do {
        ...
        try {
          ...
          const aPtr = current.and(ptr(1).not());
          const bPtr = aPtr.add(2);

          const a = Memory.readU16(aPtr);
          const b = Memory.readU16(bPtr);
          const isLdaex = a === 0xe8d4 && b === 0x2fef;
          if (isLdaex) {
            current = current.add(4);
            continue;
          }

          const isStlex = a === 0xe8c4 && b === 0x0fe1;
          if (isStlex) {
            current = current.add(4);
            continue;
          }

          puts(`0x${a.toString(16)} 0x${b.toString(16)}`);
          throw e;
        }
        const insnAddressKey = current.toString();
        const {mnemonic} = insn;

        instructionAddressKeys.push(insnAddressKey);

        const existingBlock = blocks[insnAddressKey];
        ...
        switch (mnemonic) {
          case 'b':
            branchTarget = ptr(insn.operands[0].value).or(1);
          ...
          case 'cbz':
          case 'cbnz':
            branchTarget = ptr(insn.operands[1].value).or(1);
            break;
          case 'pop.w':
            reachedEndOfBlock = insn.operands.filter(op => op.value === 'pc').length === 1;
            console.log('reachedEndOfBlock=' + reachedEndOfBlock);
            break;
          default:
            break;
        }
      ...
      if (block !== null) {
        block.end = ptr(instructionAddressKeys[instructionAddressKeys.length - 1]);

        blocks[blockAddressKey] = block;
        instructionAddressKeys.forEach(key => {
          blockByInstruction[key] = block;
        });
      }
    }
  ...
  arm64: function (buffer, pc, exceptionClearImpl, exceptionOffset, callback) {
    ...
    while (pending.length > 0) {
      ...
      do {
        ...
        const insnAddressKey = insn.address.toString();
        const {mnemonic} = insn;
    ...
    blocksOrdered.forEach(block => {
      ...
      while ((offset = relocator.readOne()) !== 0) {
        ...
        const insnAddressKey = insn.address.toString(10);
        if (branchTargets.has(insnAddressKey)) {
          writer.putLabel(insnAddressKey);
        }
...
function _getArtThreadStateTransitionImpl (vm, env) {
  ...
  Memory.patchCode(code, codeSize, buffer => {
    recompile(buffer, code, exceptionClearImpl, exceptionOffset, callback);
  });

  const perform = new NativeFunction(code, 'void', ['pointer']);
  perform._code = code;
