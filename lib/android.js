...
function findArtMethodReplacementFromQuickCode (method, thread) {
  ...
  if (!managedStack.add(managedStackOffsets.topQuickFrame).readPointer().isNull()) {
    return replacementMethod;
  }

  const linkManagedStack = managedStack.add(managedStackOffsets.link).readPointer();
  if (linkManagedStack.isNull()) {
    return replacementMethod;
  }

  const linkTopQuickFrame = linkManagedStack.add(managedStackOffsets.topQuickFrame).readPointer().and(~1);
  if (linkTopQuickFrame.isNull() || !linkTopQuickFrame.readPointer().equals(replacementMethod)) {
    return replacementMethod;
  }
