...
export function getArtClassSpec (vm) {
  ...
  vm.perform(env => {
    ...
    const readArtArrray = (objectBase, fieldOffset, lengthSize) => {
      const header = objectBase.add(fieldOffset).readPointer();
      if (header.isNull()) {
        return null;
      }

      const length = (lengthSize === 4) ? header.readU32() : header.readU64();
      if (length === 0) {
        return null;
      }
    ...
    const hasEntry = (objectBase, offset, needle, info) => {
      try {
        const artArray = readArtArrray(objectBase, offset, info.artArrayLengthSize);
        if (artArray === null) {
          return false;
        }
