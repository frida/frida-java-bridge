...
set_replacement_method (gpointer original_method,
                        gpointer replacement_method)
{
  frida_log ("set_replacement_method(%p, %p)", original_method, replacement_method);
...
delete_replacement_method (gpointer original_method)
{
  ...
  frida_log ("delete_replacement_method(%p, %p)", original_method, replacement_method);
}
                                           ...
                                           gpointer pc)
{
  ...
  {
    frida_log ("GetOatQuickMethodHeader(%p, pc=%p) => it is a replacement, returning NULL", method, pc);
    return NULL;
  }

  frida_log ("GetOatQuickMethodHeader(%p, pc=%p) => not a replacement, calling original", method, pc);
...
function maybeInstrumentGetOatQuickMethodHeaderInlineCopies () {
  ...
  for (const range of getApi().module.enumerateRanges('--x')) {
    const matches = Memory.scanSync(range.base, range.size, pattern);
    impls.push(...matches.map(({ address }) => address.sub(1)));
  }

  impls.forEach(instrumentGetOatQuickMethodHeaderInlinedCopy);

  return true;
}

function instrumentGetOatQuickMethodHeaderInlinedCopy (address) {
  ...
  console.log(`${address.sub(getApi().module.base)}: methodReg=${methodReg} scratchReg=${scratchReg}`);
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    writer.putLdrRegAddress(scratchReg, ifTrue);
    ...
    relocator.readOne();
    relocator.writeOne();
    writer.putLdrRegAddress(scratchReg, ifFalse.add(4));
    writer.putBrReg(scratchReg);

    writer.flush();
  });

  inlineHooks.push(trampoline);
  ...
  Memory.patchCode(address, 16, code => {
    ...
    writer.putBrReg(scratchReg);
    writer.flush();
    console.log('hooked, wrote:', writer.offset);
  });
}
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    console.log(`[*] Cloned ${hookedMethodId} as ${replacementMethodId}\n`);
