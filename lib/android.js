...
function makeArtQuickFrameInfoGetter (impl) {
  if (Process.arch !== 'arm64' && Process.arch !== 'arm') {
    return function () {
      throw new Error('Only supported on arm64 and arm for now');
    };
  }
...
function _getArtQuickFrameInfoGetterThunk (impl) {
  if (Process.arch == 'arm64') {
    ...
    const thunk = makeThunk(64, writer => {
      // By call convention, we pass a pointer for the result struct
      writer.putCallAddressWithArguments(impl, ['r0']);
