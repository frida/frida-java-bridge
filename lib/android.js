...
let cachedApi = null;
let everythingDeoptimized = false;
...
function _getApi () {
  ...
  if (isArt) {
    ...
    temporaryApi.artInstrumentation = artRuntime.add(runtimeOffset.instrumentation);
...
function ensureEverythingDeoptimized () {
  if (everythingDeoptimized) {
    return;
  }
  everythingDeoptimized = true;
...
function _getArtRuntimeSpec (api) {
  ...
  spec.offset.instrumentation = detectInstrumentationOffset();
...
function detectInstrumentationOffset () {
  let cur = Module.getExportByName('libart.so', '_ZN3art3Dbg22RequiresDeoptimizationEv');
...
function _getArtThreadStateTransitionImpl (vm, env) {
  ...
  const exceptionClearSymbol = Module.enumerateSymbols('libart.so').filter(s => s.name === '_ZN3art3JNI14ExceptionClearEP7_JNIEnv')[0];
  if (exceptionClearSymbol !== undefined) {
    exceptionClearImpl = exceptionClearSymbol.address;
  } else {
    const envVtable = env.handle.readPointer();
    exceptionClearImpl = envVtable.add(17 * pointerSize).readPointer();
  }
  ...
  while (pending.length > 0) {
    ...
    do {
      ...
      try {
        ...
        const first = currentAddress.readU16();
        const second = currentAddress.add(2).readU16();
