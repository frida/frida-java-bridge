...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  const mayUseCollector = (apiLevel > 28)
    ? (artHeap) => {
      const mayUseCollectorNativeFunction = Module.findExportByName('libart.so', '_ZNK3art2gc4Heap15MayUseCollectorENS0_13CollectorTypeE');
      if (mayUseCollectorNativeFunction === null) {
        return false;
      }
      const kCollectorTypeCMC = 3;
      return new NativeFunction(mayUseCollectorNativeFunction, 'bool', ['pointer', 'int'])(artHeap, kCollectorTypeCMC);
    }
    : () => false;

  if (mayUseCollector(getApi().artHeap)) {
