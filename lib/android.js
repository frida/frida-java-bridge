...
function _getApi () {
  ...
  if (isArt) {
    ...
    fixupArtQuickDeliverExceptionBug();
  }
...
function makeJdwpOptions () {
  const kJdwpTransportSocket = 2;
  const kJdwpPortFirstAvailable = 0;

  const transport = kJdwpTransportSocket;
  const server = true;
  const suspend = false;
 ...
 * complex, and essentially a wrapper around the unchecked version. As we don't
 * yet support parsing that to find the virtual call to the unchecked version,
 * we try to find the unchecked implementation through debug symbols. If that
 * fails we use the vtable, and cross our fingers that checked JNI is disabled.
 *
...
function _getArtThreadStateTransitionImpl (vm, env) {
  const artSymbols = Module.enumerateSymbols('libart.so')
      .reduce((result, symbol) => {
        result[symbol.name] = symbol.address;
        return result;
      }, {});
  const envVtable = env.handle.readPointer();
  let exceptionClearImpl = artSymbols['_ZN3art3JNI14ExceptionClearEP7_JNIEnv'];
  let nextFuncImpl = artSymbols['_ZN3art12_GLOBAL__N_18CheckJNI10FatalErrorEP7_JNIEnvPKc'];
  if (exceptionClearImpl === undefined || nextFuncImpl === undefined) {
...
function fixupArtQuickDeliverExceptionBug () {
  const prettyMethodImpl = Module.findExportByName('libart.so', '_ZN3art9ArtMethod12PrettyMethodEb')
  if (prettyMethodImpl === null) {
    return;
  }
   ...
   */
  const thisArgIndex = (pointerSize === 8) ? 0 : 1;
  let lastSeenArtMethod = null;
  Interceptor.attach(prettyMethodImpl, function (args) {
