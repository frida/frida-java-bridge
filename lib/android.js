...
`;
  ...
  const cm = new CModule(code, {
    ...
    _frida_log: new NativeCallback(messagePtr => {
      ...
      console.log('\x1b[45m[*] ' + message + '\x1b[m\n');
      Thread.sleep(0.1);
    }, 'void', ['pointer'])
  });
...
const artGetOatQuickMethodHeaderInlinedCopyHandler = {
  ...
  x64: {
    signatures: [
      {
        pattern: [
          '83 7f 08 ff', // cmp dword [rdi + 8], 0xffffffff
          '0f 84 c0 02 00 00', // je 0x41e2b6
    ...
    instrument: instrumentGetOatQuickMethodHeaderInlinedCopyX64
  }
};

function validateGetOatQuickMethodHeaderInlinedMatchArm ({ address, size }) {
  let cursor = address.add(size + 1);
  for (let i = 0; i !== 2; i++) {
    let insn;
    try {
    ...
    const { mnemonic } = insn;
    if ((mnemonic === 'ldr' || mnemonic === 'ldr.w') && insn.operands[1].value.disp === 0x14) {
      return true;
    }
...
function maybeInstrumentGetOatQuickMethodHeaderInlineCopies () {
  ...
  const signatures = handler.signatures.map(({ pattern, offset = 0, validateMatch = returnTrue }) => {
    return {
  ...
  for (const { base, size } of getApi().module.enumerateRanges('--x')) {
    for (const { pattern, offset, validateMatch } of signatures) {
      ...
      impls.push(...matches.map(({ address }) => address.sub(offset)));
    }
  }

  console.log('impls:', JSON.stringify(impls.map(p => p.sub(getApi().module.base)), null, 2));
...
function instrumentGetOatQuickMethodHeaderInlinedCopyArm (address) {
  ...
  const beq = Instruction.parse(ldr.next.add(2));
  const ifTrue = ptr(beq.operands[0].value);
  const ifFalse = beq.next.sub(1);
  console.log('ifTrue:', ifTrue.sub(getApi().module.base));
  console.log('ifFalse:', ifFalse.sub(getApi().module.base));
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    let relocator = new ThumbRelocator(address, writer);
    for (let i = 0; i !== 3; i++) {
      relocator.readOne();
    }
    relocator.writeAll();
    ...
    writer.putBCondLabel('eq', 'not_a_replacement');
    writer.putBranchAddress(ifTrue.or(1));

    writer.putLabel('not_a_replacement');
    relocator = new ThumbRelocator(ifFalse, writer);
    relocator.readOne();
    relocator.writeOne();
    writer.putBranchAddress(ifFalse.add(3));

    writer.flush();
  });

  inlineHooks.push(new InlineHook(address, 16, trampoline));

  Memory.patchCode(address, 8, code => {
    const writer = new ThumbWriter(code, { pc: address });
    writer.putLdrRegAddress('pc', trampoline.or(1));
    writer.flush();
    console.log('wrote:', writer.offset);
  });
}

function instrumentGetOatQuickMethodHeaderInlinedCopyArm64 (address) {
  const [ldrDst, ldrSrc] = Instruction.parse(address).operands;
  const scratchReg = 'x' + ldrDst.value.substring(1);
  const methodReg = ldrSrc.value.base;
  const ifTrue = ptr(Instruction.parse(address.add(8)).operands[0].value);
  const ifFalse = address.add(12);
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    let relocator = new Arm64Relocator(address, writer);
    for (let i = 0; i !== 3; i++) {
      relocator.readOne();
    }
    relocator.writeAll();
    ...
    writer.putBCondLabel('eq', 'not_a_replacement');
    writer.putBranchAddress(ifTrue);

    writer.putLabel('not_a_replacement');
    relocator = new Arm64Relocator(ifFalse, writer);
    relocator.readOne();
    relocator.writeOne();
    writer.putBranchAddress(ifFalse.add(4));
...
function instrumentGetOatQuickMethodHeaderInlinedCopyX64 (address) {
}
