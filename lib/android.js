...
function ensureArtKnowsHowToHandleHookedMethods () {
  ...
  const getOatQuickMethodHeader = Module.findExportByName('libart.so', '_ZN3art9ArtMethod23GetOatQuickMethodHeaderEm');
  if (getOatQuickMethodHeader === null) {
    return;
  }

  Interceptor.attach(getOatQuickMethodHeader, {
    onEnter(args) {
      const method = args[0].toString();
      this.isHookedMethod = hookedArtMethods[method.toString()] !== undefined;
    },
    onLeave(retval) {
      if (this.isHookedMethod) {
        retval.replace(NULL);
      }
    }
  });
