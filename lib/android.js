...
function tryGetEnvJvmti (vm, runtime) {
  ...
  vm.perform(() => {
    const t1 = Date.now();
    const ensurePluginLoadedAddr = findArtEnsurePluginMatbrik();
    const t2 = Date.now();
    console.log(`Found EnsurePluginLoaded in ${t2 - t1} ms`);
...
function findArtEnsurePluginMatbrik () {
  const impl = Module.findExportByName('libart.so', '_ZN3art7Runtime18EnsurePluginLoadedEPKcPNSt3__112basic_stringIcNS3_11char_traitsIcEENS3_9allocatorIcEEEE');
  if (impl !== null) {
    return impl;
  }

  const results = findString('libopenjdkjvmti.so', 'libart.so');
  const pattern = makeArm64PcRelAddPattern(results[0].address, 'x1');
  const patternResults = findPattern(pattern, 'libart.so');
  return patternResults.map((match) => {
    ...
    if (disasm.mnemonic === 'b') {
      return ptr(disasm.operands[0].value);
    }
    return null;
  }).filter(element => element !== null).flat()[0];
}

function findArtEnsurePluginHackcatml () {
  const impl = Module.findExportByName('libart.so', '_ZN3art7Runtime18EnsurePluginLoadedEPKcPNSt3__112basic_stringIcNS3_11char_traitsIcEENS3_9allocatorIcEEEE');
  if (impl !== null) {
    return impl;
  }
  ...
  if (Process.arch === 'arm64') {
    return findPcRelativeReferenceToString('libart.so', '?1 ?? ff ?0 21 ?? ?? 91', 0, libopenjdkjvmtiSoAddr,
    ...
    return findPcRelativeReferenceToString('libart.so', '49 0d f1 ?? 0a', -1, libopenjdkjvmtiSoAddr,
      match => scanForward(match.last.next, 4, insn => {
...
function findPattern (pattern, moduleName) {
  const module = Process.getModuleByName(moduleName);
...
function makeArm64PcRelAddPattern (address, register) {
  const buf = Memory.alloc(4);
  const cw = new Arm64Writer(buf);
  cw.putAddRegRegImm(register, register, address.and(0xfff).toUInt32());
  const code = new Uint8Array(ArrayBuffer.wrap(buf, 4));
  return Array.from(code).map(toHexByte).join(' ');
}
...
function stringToBytesHex (str) {
  ...
  const n = str.length;
  for (let i = 0; i !== n; i++) {
    bytes.push(toHexByte(str.charCodeAt(i)));
  }
  return bytes.join(' ');
}
...
function _getArtRuntimeSpec (api) {
  ...
  const isApiLevel34OrApexEquivalent = Module.findExportByName('libart.so', '_ZN3art7AppInfo29GetPrimaryApkReferenceProfileEv') !== null ||
    Module.findExportByName('libart.so', '_ZN3art6Thread15RunFlipFunctionEPS0_') !== null;
...
function instrumentArtMethodInvocationFromInterpreter () {
  ...
  if (!doCallFound && Process.arch === 'arm64') {
    const invokingPercentSString = '49 6e 76 6f 6b 69 6e 67 20 25 73';
    ...
    if (invokingPercentSAddr !== null) {
      ...
      const doCallFuncAddrs = findPcRelativeReferenceToString('libart.so', '?2 e? ff ?0 42 ?? ?? 91', 0, invokingPercentSAddr,
        match => {
          ...
          return (foundFuncs.length === 2) ? foundFuncs : null;
        }
      ) ?? [];
      for (const address of doCallFuncAddrs) {
...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  let runFlip = null;
  runFlip = Module.findExportByName('libart.so', '_ZN3art6Thread15RunFlipFunctionEPS0_');
  if (runFlip === null) {
...
function makeArtThreadStateTransitionImpl (vm, env, callback) {
  ...
  if (fatalErrorSymbol === undefined && Process.arch === 'arm64') {
    ...
    let adrpAddInJNITrueFatalErrorFunc;
    let jniTrueFatalErrorFunc;
    let jniExceptionClearFunc;
    ...
    if (jniFatalErrorCalledAddr !== null) {
      ...
      if (findPcRelativeReferenceToString('libart.so', '?1 ?? ff ?0 21 ?? ?? 91', 0, jniFatalErrorCalledAddr,
        match => {
          ...
          adrpAddInJNITrueFatalErrorFunc = address;

          jniTrueFatalErrorFunc = scanBackwardForSubStp(adrpAddInJNITrueFatalErrorFunc);
          jniExceptionClearFunc = scanBackwardForSubStp(jniTrueFatalErrorFunc.sub(4));

          return occurrences;
        }
      ) !== null) {
        exceptionClearImpl = jniExceptionClearFunc;
        nextFuncImpl = jniTrueFatalErrorFunc;
      }
    }
  }
...
function findStringInRodata (modName, stringBytes) {
  const rodataSection = Module.enumerateSections(modName).find(s => s.name === '.rodata');
  const matches = Memory.scanSync(rodataSection.address, rodataSection.size, stringBytes);
  if (matches.length === 0) {
    return null;
  }
