...
function _getApi () {
  ...
  const pending = isArt ? [{
    ...
    functions: {
      ...
      '_ZN3art9JavaVMExt12DecodeGlobalEPv': ['art::JavaVMExt::DecodeGlobal', 'pointer', ['pointer', 'pointer']],
...
function _getArtRuntimeSpec (api) {
   ...
   * InternTable* intern_table_;      <-- and these
  ...
  for (let offset = startOffset; offset !== endOffset; offset += pointerSize) {
    ...
    if (value.equals(vm)) {
      ...
      let heapOffset = internTableOffset - (5 * pointerSize);
      if (apiLevel >= 23) {
