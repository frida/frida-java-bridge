...
function parseArm64InstrumentationPointer (insn) {
  ...
  const dst = ops[0];
  if (dst.type !== 'reg' || dst.value === 'x0' || dst.value === 'sp') {
    return null;
  }
  const isThisBase = (r) => r === 'x0' || r === 'x19';
  const mem = ops[1].value;
  if (!mem || !isThisBase(mem.base)) {
    return null;
  }

  const off = (mem.disp ?? mem.offset ?? 0);
