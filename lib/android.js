...
function _getArtApexVersion () {
  try {
    ...
    for (const line of mountInfo.split('\n')) {
      ...
      if (mountRoot === undefined || !mountRoot.startsWith('/apex/com.android.art')) continue;
      const mountSource = elements[10];
      if (mountRoot.includes('@')) {
        sourceVersionMap[mountSource] = mountRoot.split('@')[1];
      } else {
        artSource = mountSource;
      }
    }
    const strVersion = sourceVersionMap[artSource];
    return strVersion !== undefined ? parseInt(strVersion) : getAndroidApiLevel() * 10_000_000;
  } catch {
