...
const makeCxxNativeFunctionReturningPointer =
    (Process.arch === 'ia32')
    ? makeCxxNativeFunctionReturningPointerInFirstArg
    : makeCxxNativeFunctionReturningPointerGeneric;
  ...
  const pending = isArt ? [{
    ...
    functions: {
      ...
      '_ZN3art9JavaVMExt12DecodeGlobalEPv': function (address) {
        const decodeGlobal = makeCxxNativeFunctionReturningPointer(address, ['pointer', 'pointer']);
...
const threadStateTransitionRecompilers = {
  ia32: function (buffer, pc, exceptionClearImpl, exceptionOffset, callback) {
    const blocks = {};
      ...
      while ((offset = relocator.readOne()) !== 0) {
        ...
        switch (mnemonic) {
          ...
          case 'mov': {
            ...
            if (dst.type === 'mem' && dst.value.disp === exceptionOffset &&
                src.type === 'imm' && src.value === 0) {
              writer.putPushfx();
              writer.putPushax();
              writer.putMovRegReg('ebp', 'esp');
              writer.putAndRegU32('esp', 0xfffffff0);
              writer.putCallAddressWithAlignedArguments(callback, [dst.value.base]);
        ...
        switch (mnemonic) {
          ...
          case 'str.w': {
            ...
            if (ops[1].value.disp === exceptionOffset) {
              const threadReg = ops[1].value.base;
...
function makeCxxNativeFunctionReturningPointerGeneric (address, argTypes) {
  return new NativeFunction(address, 'pointer', argTypes);
}

function makeCxxNativeFunctionReturningPointerInFirstArg (address, argTypes) {
  const impl = new NativeFunction(address, 'void', ['pointer'].concat(argTypes));
  return function () {
    const resultPtr = Memory.alloc(pointerSize);
    impl.apply(impl, [resultPtr].concat(Array.prototype.slice.call(arguments)));
