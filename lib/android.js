...
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_X64 = 14;
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_ARM = 8;
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_ARM64 = 16;
...
function instrumentArtQuickEntrypoints (vm) {
  ...
  if (api.artNterpEntryPoint !== undefined && !api.artNterpEntryPoint.equals(NULL)) {
    quickEntrypoints.push(api.artNterpEntryPoint);
  }
  ...
  Module.enumerateExports('libart.so')
    ...
    .forEach(exp => {
      Interceptor.attach(exp.address, function (args) {
        ...
        const replacementMethod = replacedMethods.get(method.toString());
...
function findArtMethodReplacementFromQuickCode (method, thread) {
  const replacementMethod = replacedMethods.get(method.toString());
  if (replacementMethod === undefined) {
    return NULL;
  }
...
const findArtMethodReplacementFromQuickCodeImpl = new NativeCallback(
    findArtMethodReplacementFromQuickCode, 'pointer', ['pointer', 'pointer']
);
...
function writeArtQuickCodeReplacementTrampolineIA32 (trampoline, target, vm) {
  ...
  Memory.patchCode(trampoline, 128, code => {
    ...
    writer.putCallAddressWithAlignedArguments(findArtMethodReplacementFromQuickCodeImpl, ['eax', 'ebx']);
...
function writeArtQuickCodeReplacementTrampolineX64 (trampoline, target, vm) {
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    writer.putCallAddressWithAlignedArguments(findArtMethodReplacementFromQuickCodeImpl, ['rdi', 'rbx']);
...
function writeArtQuickCodeReplacementTrampolineArm (trampoline, target, vm) {
  const artMethodOffsets = getArtMethodSpec(vm).offset;
  ...
  Memory.patchCode(trampoline, 128, code => {
    ...
    writer.putCallAddressWithArguments(findArtMethodReplacementFromQuickCodeImpl, ['r0', 'r9']);
...
function writeArtQuickCodeReplacementTrampolineArm64 (trampoline, target, vm) {
  const artMethodOffsets = getArtMethodSpec(vm).offset;
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    writer.putCallAddressWithArguments(findArtMethodReplacementFromQuickCodeImpl, ['x0', 'x19']);
    ...
    if (!relocator.eoi) {
      writer.putBranchAddress(target.add(offset));
    }
...
function writeArtQuickCodePrologueArm (target, trampoline) {
  const targetAddress = target.and(THUMB_BIT_REMOVAL_MASK);
...
function writeArtQuickCodePrologueArm64 (target, trampoline) {
  Memory.patchCode(target, 16, code => {
    const writer = new Arm64Writer(code, { pc: target });

    writer.putBranchAddress(trampoline);
    writer.flush();
  });
}
...
class ArtQuickCodeInterceptor {
  ...
  activate (vm) {
    ...
    const quickCodeAddress = (Process.arch === 'arm')
      ? this.quickCode.and(THUMB_BIT_REMOVAL_MASK)
      : this.quickCode;

    this.overwrittenPrologue = Memory.dup(quickCodeAddress, prologueLength);
  ...
  deactivate () {
    ...
    const prologueLength = this.overwrittenPrologueLength;
    ...
    Memory.patchCode(quickCodeAddress, prologueLength, code => {
      ...
      writer.putBytes(this.overwrittenPrologue.readByteArray(prologueLength));
      writer.flush();
    });
...
function isArtQuickEntrypoint (address) {
  ...
  return address.equals(api.artClassLinker.quickGenericJniTrampoline) ||
      ...
      address.equals(api.artClassLinker.quickResolutionTrampoline) ||
      address.equals(api.artClassLinker.quickImtConflictTrampoline) ||
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    if (api.artNterpEntryPoint !== undefined && quickCode.equals(api.artNterpEntryPoint)) {
      patchArtMethod(hookedMethodId, {
    ...
    if (!isArtQuickEntrypoint(quickCode)) {
      this.interceptor = new ArtQuickCodeInterceptor(quickCode);
      this.interceptor.activate(vm);
    }

    replacedMethods.set(hookedMethodId.toString(), replacementMethodId);

    notifyArtMethodHooked(hookedMethodId, vm);
  }

  revert (vm) {
    patchArtMethod(this.hookedMethodId, this.originalMethod, vm);

    replacedMethods.delete(this.hookedMethodId.toString());
