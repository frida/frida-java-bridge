...
const threadStateTransitionRecompilers = {
  arm64: function (buffer, pc, exceptionClearImpl, exceptionOffset, callback) {
    ...
    writer.flush();
    console.log(`\n\n\n\nGenerated ${writer.offset} bytes of machine code\n\n\n\n`);

    writer.dispose();
  }
};

global.runExperiment = function () {
  addArtThreadStateTransitionFunctions(getApi());
};
...
function addArtThreadStateTransitionFunctions (api) {
  ...
  vm.perform(() => {
    ...
    const performTransition = new NativeFunction(code, 'void', ['pointer']);

    console.log('Taking it for a spin on env=' + env.handle + ' thread=' + getArtThreadFromEnv(env));
    try {
      performTransition(env.handle);
      console.log('Survived!');
    } catch (e) {
      console.log('Fail: ' + e);
    }

    api._stateTransitionData = { code, callback };
  });
}

function onThreadStateTransitionComplete (thread) {
  console.log('onThreadStateTransitionComplete thread=' + thread);
}
