...
function instrumentArtQuickEntrypoints (vm) {
  ...
  if (api.artNterpEntryPoint !== undefined) {
    quickTrampolines.push(api.artNterpEntryPoint);
  }

  quickTrampolines.forEach(trampoline => {
    const interceptor = new ArtQuickCodeInterceptor(trampoline);
...
function isArtQuickEntrypoint (address) {
  ...
  return address.equals(api.artClassLinker.quickGenericJniTrampoline) ||
      ...
      address.equals(api.artNterpEntryPoint);
}
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    if (quickCode.equals(api.artNterpEntryPoint)) {
      patchArtMethod(hookedMethodId, {
        quickCode: api.artClassLinker.quickToInterpreterBridgeTrampoline
      }, vm);
    }
