...
function getApi () {
  ...
  const pending = temporaryApi.flavor === 'art' ? [{
    ...
    extraInit: function(exports, fullAPI) {
      fullAPI.isXposed = '_ZNK3art9OatHeader23IsXposedOatVersionValidEv' in exports;
      let exp = exports[fullAPI.isXposed ? '_ZN3art6mirror6Object5CloneEPNS_6ThreadEm' :
       '_ZN3art6mirror6Object5CloneEPNS_6ThreadE'];
      if (exp !== undefined) {
        fullAPI['art::mirror::Object::Clone'] = new NativeFunction(exp.address, 'pointer', 
          fullAPI.isXposed ? ['pointer', 'pointer', 'int'] : ['pointer', 'pointer']);
        return true;
      }
      return false;
    }
  ...
  pending.forEach(function (api) {
    ...
    remaining += Object.keys(functions).length + Object.keys(variables).length + (api.extraInit ? 1 : 0);
    ...
    if (api.extraInit && api.extraInit(exportByName, temporaryApi)) {
      remaining--;
    }
  });
