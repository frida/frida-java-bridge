...
function maybeInstrumentGetOatQuickMethodHeaderInlineCopies () {
  ...
  const base = getApi().module.base;
  const x = impls.map(({ address, size, validationResult }) => {
    return {
      address: address.sub(base),
      size: size,
      validationResult
    };
  });
  console.log(`Instrumenting ${x.length} targets: ${JSON.stringify(x, null, 2)}`);
...
function instrumentGetOatQuickMethodHeaderInlinedCopyArm64 ({ address, size, validationResult }) {
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    relocator.readOne();
    const tailInstructionAddress = relocator.input.address;
    ...
    const tailIsRegular = tailInstructionAddress.equals(target.whenRegularMethod);

    writer.putLabel(tailIsRegular ? 'regular_method' : 'runtime_or_replacement_method');
    relocator.writeOne();
    writer.putBranchAddress(tailInstructionAddress.add(4));
