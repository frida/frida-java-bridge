...
`;
  ...
  const cm = new CModule(code, {
    ...
    get_oat_quick_method_header_impl: ptr(1), //getOatQuickMethodHeaderImpl ?? ptr('0xdeadbeef'),
...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  if (apiLevel >= 31 && Process.arch === 'arm64') {
    // GetOatQuickMethodHeader() is inlined in a few places :(
    ...
    for (const range of getApi().module.enumerateRanges('--x')) {
      console.log('Processing range:', JSON.stringify(range));
