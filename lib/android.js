  ...
  arm: function (buffer, pc, exceptionClearImpl, exceptionOffset, callback) {
    ...
    while (pending.length > 0) {
      ...
      do {
        ...
        try {
          ...
          puts(`0x${first.toString(16)} 0x${second.toString(16)}`);
          throw e;
        }
        const {mnemonic} = insn;
    ...
    blocksOrdered.forEach(block => {
      ...
      do {
        ...
        switch (mnemonic) {
          case 'b':
            writer.putBLabel(ptr(insn.operands[0].value).toString());
            relocator.skipOne();
            break;
          case 'beq.w':
            writer.putBeqLabel(ptr(insn.operands[0].value).toString());
            relocator.skipOne();
            break;
          case 'beq':
          case 'bne':
          case 'bgt':
            writer.putBCondLabel(mnemonic.substr(1), ptr(insn.operands[0].value).toString());
            relocator.skipOne();
            break;
          case 'cbz': {
            const ops = insn.operands;
            writer.putCbzRegLabel(ops[0].value, ptr(ops[1].value).toString());
            relocator.skipOne();
            break;
          }
          case 'cbnz': {
            const ops = insn.operands;
            writer.putCbnzRegLabel(ops[0].value, ptr(ops[1].value).toString());
            relocator.skipOne();
            break;
          }
          default:
    ...
    writer.flush();
    console.log('Wrote ' + writer.offset + ' bytes');
    writer.dispose();
