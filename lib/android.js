...
const threadStateTransitionRecompilers = {
  ia32: function (buffer, pc, exceptionClearImpl, exceptionOffset, neuteredOffsets, callback) {
        ...
        switch (mnemonic) {
          ...
          case 'mov': {
            ...
            if (dst.type === 'mem' && src.type === 'imm') {
              const dstOffset = dst.value.disp;

              if (dstOffset === exceptionOffset && src.value === 0) {
                writer.putPushfx();
                writer.putPushax();
              ...
              if (neuteredOffsets.has(dstOffset)) {
                relocator.skipOne();
  ...
  arm: function (buffer, pc, exceptionClearImpl, exceptionOffset, neuteredOffsets, callback) {
    const blocks = {};
    const blockByInstruction = {};
    const branchTargets = new Set();
  ...
  arm64: function (buffer, pc, exceptionClearImpl, exceptionOffset, neuteredOffsets, callback) {
    const blocks = {};
    const blockByInstruction = {};
    const branchTargets = new Set();
