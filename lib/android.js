...
function maybeInstrumentGetOatQuickMethodHeaderInlineCopies () {
  ...
  for (const { base, size } of getApi().module.enumerateRanges('--x')) {
    for (const { pattern, offset, validateMatch } of signatures) {
      ...
      impls.push(...matches.map(({ address, size }) => {
...
function instrumentGetOatQuickMethodHeaderInlinedCopyArm ({ address, size }) {
  ...
  Memory.patchCode(trampoline, 256, code => {
    ...
    do {
      ...
      redirectCapacity = size - 2 + offset;
    } while (redirectCapacity < 10 && !relocator.eoi);
    relocator.writeAll();
    writer.putBranchAddress(ifFalse.add(offset + 1));
...
function instrumentGetOatQuickMethodHeaderInlinedCopyArm64 ({ address, size }) {
