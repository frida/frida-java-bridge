...
class ArtMethodMangler {
  ...
  resolveTarget (vm) {
    ...
    patchArtMethod(target, this.originalMethod, vm);
    return target;
  }
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    if (this.originalMethod === null) {
      ...
      if ((this.originalMethod.accessFlags & kAccXposedHookedMethod) !== 0 && xposedIsSupported()) {
        ...
        this.originalMethod = fetchArtMethod(this.hookedMethod, vm);
      }
    }
    ...
    patchArtMethod(hookedMethod, {
      'jniCode': impl,
      'accessFlags': (hookedMethod.add(artMethodOffset.accessFlags).readU32() | kAccNative | kAccFastNative) >>> 0,
      'quickCode': api.artQuickGenericJniTrampoline,
      'interpreterCode': api.artInterpreterToCompiledCodeBridge
    }, vm);

    notifyArtMethodHooked(hookedMethod);
  }
...
class DalvikMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    if (this.originalMethod === null) {
      ...
      this.targetMethod = Memory.dup(methodId, DVM_METHOD_SIZE);
    }
  ...
  synchronizeVtable (wrapper, isInstanceMethod, env, api) {
    if (this.originalMethod === null) {
      return; // nothing to do -- implementation hasn't been replaced
    }

    const thread = env.handle.add(DVM_JNI_ENV_OFFSET_SELF).readPointer();
    const objectPtr = api.dvmDecodeIndirectRef(thread, isInstanceMethod ? wrapper.$handle : wrapper.$getClassHandle(env));
    let classObject;
    if (isInstanceMethod) {
    ...
    let entry = patchedClasses.get(key);
    if (entry === undefined) {
      ...
      patchedClasses.set(key, entry);
    }

    key = this.methodId.toString(16);
    if (!entry.targetMethods.has(key)) {
      const methodIndex = entry.shadowVtableCount++;
      entry.shadowVtable.add(methodIndex * pointerSize).writePointer(this.targetMethod);
      this.targetMethod.add(DVM_METHOD_OFFSET_METHOD_INDEX).writeU16(methodIndex);
      entry.vtableCountPtr.writeS32(entry.shadowVtableCount);

      entry.targetMethods.add(key);
    }
  }
}
